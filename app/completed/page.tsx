...
const Completed = () => {
  ...
  const showValue = (id: any) => {
    const singleTask = complete.find((task: any) => task.id === id);
    if (singleTask) {
  ...
  const getTask = async () => {
    try {
      ...
      const userTasks = completedTask.filter(
        (task: any) => task.userId === userId
      );
      if (JSON.stringify(userTasks) !== JSON.stringify(complete)) {
        setcomplete(userTasks);
      }
    } catch (error) {
  ...
  return (
    <div className="card w-100 bg-base-100 shadow-xl min-h-screen p-5">
      ...
      <div className="main-task grid mdl:grid-cols-2 grid-cols-1 gap-5 mt-5">
        {complete?.map((task: any, index: any) => (

...
const DoItNow = () => {
  ...
  const showValue = (id: any) => {
    const singleTask = inComplete.find((task: any) => task.id === id);
    if (singleTask) {
  ...
  const getTask = async () => {
    try {
      ...
      const inCompleteTask = data.filter(
        (task: any) => task.isCompleted === false
      );
      const userTasks = inCompleteTask.filter(
        (task: any) => task.userId === userId
      );
      if (JSON.stringify(userTasks) !== JSON.stringify(inComplete)) {
        setInComplete(userTasks);
      }
    } catch (error) {
  ...
  return (
    <div className="card w-100 bg-base-100 shadow-xl min-h-screen p-5">
      ...
      <div className="flex justify-between items-center mb-5">
        <h1 className="text-3xl">Incompleted Task</h1>
        <MdAddCircleOutline
          onClick={() =>
      ...
      <div className="main-task grid mdl:grid-cols-2 grid-cols-1 gap-5 mt-5">
        {inComplete?.map((task: any, index: any) => (
                ...
                <div className="modal-box">
                  ...
                  <div className="form-control">
                    <label className="label cursor-pointer">
                      ...
                      <input
                        onChange={(e) =>
                          setUpdateIsImportant(!updateIsImportant)
                        }
                        type="checkbox"
                        className="checkbox"
